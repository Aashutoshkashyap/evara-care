  ...
  return (
    <section id="faq" className="max-w-3xl mx-auto px-8 py-20">
      ...
      <div className="text-center mb-14">
        <h2 className="text-3xl md:text-4xl font-bold text-[#0e142a] mb-3">
        ...
        {faqs.map((faq, index) => (
          <div
            ...
            className={`border rounded-xl overflow-hidden transition-colors ${
              openIndex === index ? "border-[#0e142a]/30 bg-[#0e142a]/5" : "border-gray-200 bg-white"
            }`}
          >
            <button
            ...
            >
              <span className={`font-semibold text-[15px] pr-4 ${openIndex === index ? "text-[#0e142a]" : "text-[#111]"}`}>
                {faq.question}
              </span>
              <ChevronDown
                size={20}
                className={`flex-shrink-0 transition-transform duration-300 ${
                  openIndex === index ? "rotate-180 text-[#0e142a]" : "text-gray-400"
                }`}
              />
            </button>
            <div
